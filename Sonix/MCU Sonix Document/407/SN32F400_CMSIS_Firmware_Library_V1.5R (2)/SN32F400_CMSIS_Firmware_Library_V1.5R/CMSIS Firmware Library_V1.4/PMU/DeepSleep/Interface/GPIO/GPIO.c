...
*	1.0				2024/11/26	SA1			1. First release
*
*____________________________________________________________________________
...
extern volatile uint32_t	wP1_DS_WKP_Int_Times;
...
void GPIO_Init(void)
{
	...
	//P2.2 as Input Inactive schmitt disable
	GPIO_Mode			(GPIO_PORT2, GPIO_PIN2, GPIO_MODE_INPUT);
	GPIO_Config		(GPIO_PORT2, GPIO_PIN2, GPIO_CFG_INACTIVE_SCHMITT_DIS);	
	//P2.2 as both edge
...
* Description	: set GPIO0 as edge or level trigger
* Input			: pin_number	- GPIO_PIN0~7, 10~11
							is					-	0: edge sensitive
														1: event sensitive
							ibs					-	0: edge trigger
														1: both edge trigger
...
* Description	: set GPIO1 as edge or level trigger
* Input			: pin_number	- GPIO_PIN0, 1, 2, ...,12
							is					-	0: edge sensitive
														1: event sensitive
							ibs					-	0: edge trigger
														1: both edge trigger
...
* Description	: set GPIO2 as edge or level trigger
* Input			: pin_number	- GPIO_PIN0, 1, 2, ...,10
							is					-	0: edge sensitive
														1: event sensitive
							ibs					-	0: edge trigger
														1: both edge trigger
...
* Description	: set GPIO3 as edge or level trigger
* Input			: pin_number	- GPIO_PIN0~2, 5~13
							is					-	0: edge sensitive
														1: event sensitive
							ibs					-	0: edge trigger
														1: both edge trigger
...
void GPIO_Interrupt(uint32_t port_number, uint32_t pin_number, uint32_t enable)
{
	switch (port_number)
	{
		...
		case GPIO_PORT3:
			//check SWD pin
			if ((pin_number == GPIO_PIN5) || (pin_number == GPIO_PIN6))
			{
				if(SN_SYS0->SWDCTRL != 0x1) 
				{
					return;
				}
			}
			...
			if (pin_number == GPIO_PIN7)
			{
				if(SN_SYS0->EXRSTCTRL != 0x1)
				{
					return;
				}
			}
			SN_GPIO3->IC = 0x000FFFFF;
...
* Input			: port_number - GPIO0, GPIO1, GPIO2, GPIO3
							...
							value				-	0: Pull-up enable 
														1: Pull-down enable
														2: Inactive (no pull-up resistor enabled, Schmitt trigger enabled)
														3: Inactive (no pull-up resistor enabled, Schmitt trigger disabled)
* Output		: None
* Return		: None
* Note			: None
*****************************************************************************/
...
{
	wP1_DS_WKP_Int_Times++;
	
	SN_GPIO1->IS = 0x380;                       //Set P1 as Event sensitive
	...
	else if (GPIO_IntStatus(GPIO_PORT1,GPIO_PIN7) == 1)
	{
		...
		if (wP1_DS_WKP_Int_Times == 1)
    {
			SN_GPIO2->BSET = 1;                   //P2.0=High
			SN_GPIO2->BCLR = 1;                   //P2.0=Low
    }
	}
	else if (GPIO_IntStatus(GPIO_PORT1,GPIO_PIN8) == 1)
	{
		...
		if (wP1_DS_WKP_Int_Times == 1)
    {
			SN_GPIO2->BSET = 0x2;                   //P2.1=High
			SN_GPIO2->BCLR = 0x2;                   //P2.1=Low
    }
	}
	else if (GPIO_IntStatus(GPIO_PORT1,GPIO_PIN9) == 1)
	{
		...
		if (wP1_DS_WKP_Int_Times == 1)
    {
			SN_GPIO2->BSET = 0x4;                 //P2.2=High
			SN_GPIO2->BCLR = 0x4;                 //P2.2=Low
    }
	}
