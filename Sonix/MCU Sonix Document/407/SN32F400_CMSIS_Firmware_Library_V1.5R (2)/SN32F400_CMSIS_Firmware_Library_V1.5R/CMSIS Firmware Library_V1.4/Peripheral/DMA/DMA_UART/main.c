...
void DMAUart0TXInit(void);
void DMAUart0RXInit(void);

void DMAUart0TXStart(void);
void DMAUart0RXStart(void);
...
uint8_t b_aryUartTXData[DMA_TEST_SIZE];
uint8_t b_aryUartRXData[DMA_TEST_SIZE];
...
/*****************************************************************************
* Function		: main
* Description	: Demo code of DMA UART.
* Input			: None
* Output		: None
* Return		: None
* Note			: Connect UART0 TX to UART0 RX
*****************************************************************************/
int	main(void)
{
	uint32_t i;
	...
	// Disable UART ISR when using DMA UART
	...
	for(i = 0; i<DMA_TEST_SIZE ;i++)
	{
		...
		b_aryUartTXData[i] = i & 0xFF;
	}
	
	// Start DMA UART RX transfer before UART TX
	UART0_DMAEnable();
	...
	for(i = 0; i<DMA_TEST_SIZE ;i++)
	{
		if(b_aryUartTXData[i] != b_aryUartRXData[i])
			while(1); //Fail
	}
...
/*****************************************************************************
* Function		: DMAUart0TXInit
* Description	: Initial DMA & UART for memory to peripheral(UART TX) transfer 
* Input				: None
* Output			: None
* Return			: None
* Note				: None
...
void DMAUart0TXInit(void)
{
	...
	// Destination : Peripheral UART, Fixed, 8bit, UART0 TX
	...
	stDMACH_Init.b_DstReqSel   = DMA_DSTRS_UART0_TX;
...
/*****************************************************************************
* Function		: DMAUart0RXInit
* Description	: Initial DMA for peripheral(UART RX) to memory transfer 
* Input				: None
* Output			: None
* Return			: None
* Note				: None
...
void DMAUart0RXInit(void)
{
	...
	// Set DMA CH1 as P(UART RX) to M(SRAM) mode
...
{	
	...
	stDMACH_Start.w_SrcAddr 	= (uint32_t)b_aryUartTXData;
	stDMACH_Start.w_DstAddr	 	= (uint32_t)&SN_UART0->TH;
...
{	
	...
	//Set source/destination address and total size
	stDMAStart.w_SrcAddr 	 = (uint32_t)&SN_UART0->RB;
	stDMAStart.w_DstAddr	 = (uint32_t)b_aryUartRXData;
