...
int	main(void)
{
	...
	while (1)
	{
		...
		if(read_key)										// key event occur
		{
			...
			//right move old key code
			segment_buff[0] = segment_buff[1];
			segment_buff[1] = segment_buff[2];
			segment_buff[2] = segment_buff[3];
			
			//refresh new key code
			segment_buff[3] = SEGMENT_TABLE[i];
		}
		Digital_Scan();
		UT_DelayNx10us(100);									
	}
