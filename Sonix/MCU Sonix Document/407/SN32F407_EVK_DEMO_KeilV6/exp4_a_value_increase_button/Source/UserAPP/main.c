...
int	main(void)
{
	...
	Digital_DisplayDEC(increase_value);
	while (1)
	{
		__WDT_FEED_VALUE;
		
		read_key = single_level_detect();						//read key action
		if(read_key)		//key push event
		{
			increase_value = increase_value < 9999 ? increase_value+1 : 0;
			Digital_DisplayDEC(increase_value);
		}
		Digital_Scan();
		UT_DelayNx10us(100);									
