...
int	main(void)
{
	...
	ADC_FuncInit(ADC_ADCKS_DIV1,ADC_ADLEN_12BIT,ADC_CH_SINGLE_CHANNEL,ADC_SCMODE_SINGLE_MODE);		//set single channel mode,channel AIN0
	
	//set channel
	SN_ADC->CONVCTRL_b.CHS0 = 1;
	
	SN_ADC->ADM_b.ADS = 1;		//START ADC SAMPLE
	CT16B0_Init();
	while (1)
	{
		__WDT_FEED_VALUE;
		if(timer_1ms_flag)
		{
			...
			SN_ADC->ADM_b.ADS = 1;		//adc sample 1tims/ms

		}
		if(SN_ADC->ADM_b.EOC)	//end of convert
		{
			adb = SN_ADC->ADB & 0xfff;
			SN_ADC->ADM_b.ADS = 1;		//START ADC SAMPLE
			
			adc_sample += (int16_t)(adb - adc_sample) >> 3;		//low pass filter
			
			Digital_DisplayDEC(adc_sample);
		}
